...
class BasicGraphs(Basics):
	...
	def __init__(
		...
		super().__init__(matplotlib_style, code_font_family=code_font_family, text_font_family=text_font_family,
		                 color_scheme=color_scheme)

	def line_plot(self, x, y, title, subtitle, xlabel, ylabel, color_scheme, size=(10, 6)) -> tuple:
		...
		plt.text(
			x=0.125, y=0.90, s=title, fontname=self.text_font_family,
			fontsize=24, ha='left', transform=fig.transFigure
		)

		plt.text(
			x=0.125, y=0.86, s=subtitle, fontname=self.text_font_family,
